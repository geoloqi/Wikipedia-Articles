...
info['locations'].each do |location|
  ...
  data['results'].each{|article|
    ...
    if pages.include? article['title']+"\n" 
      print '-'
      next
    end

    pages.push article['title']+"\n"
  
    # puts article['url'] + ' ' + article['abstract']
    
    # Figure out an appropriate radius
    radius = 400
    
    if article['abstract'] =~ /is an? [\w ]*neighborhood|is an? \w* ?area/
      radius = 800
    elsif article['abstract'] =~ /is an? \w* ?park/
      radius = 400
    elsif article['abstract'] =~ /is an? \w* ?building/
      radius = 120
    end
